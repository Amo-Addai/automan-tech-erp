...
var externalMonitoringHandler = require('./MONITORING_HANDLERS/ExternalMonitoringHandlersFunctions.js');

var sourceType = "External", defaultSource = null;
...
var ExternalAutoAuditingFunctions = {

    preprocessAutoAuditData: function (data, extra) { // extra properties: .name .details .autoaudit & .source
        try {
            if (!data || !extra) return null;

            console.log("PROCESSING EXTERNAL AUTOAUDIT DATA!!!")

            // PREPROCESS DATA BASED ON EXTERNAL SOURCE (SOFTWARE - BRO / OSSEC / KISMET / LYNIS)
            // data MUST INCLUDE ALL REQUIRED PROPERTIES PERTAINING TO THE PARTICULAR AUTOAUDIT TO BE USED FOR THE AUTOEVENT
            // return externalMonitoringHandler[extra.autoaudit].preprocessAutoAuditData(data, extra);
            ...
            // WITH EXTERNAL, YOU CAN ALSO PREPROCESS DATA BASED ON THE SOURCE
            return externalMonitoringHandler[extra.autoaudit].preprocessAutoAuditData(data, extra);
        } catch (err){
            console.log("ERROR -> " + JSON.stringify(err));
    ...
    testAutoLogsForAutoEvents: async function (autolog, relatedAutologs) {
        return new Promise(async (resolve, reject) => {
            try {
                console.log("EXTERNAL TESTING HAPPENING NOW ...")
                var x = await externalMonitoringHandler[autolog.autoaudit].testAutoLogsForAutoEvents(autolog, relatedAutologs);
    ...
    preprocessAutoEventDataStraightAway: function (data, extra) {
        try {
            ...
            console.log("PROCESSING EXTERNAL AUTOEVENT DATA STRAIGHT AWAY !!!")

            // data MUST INCLUDE ALL REQUIRED PROPERTIES PERTAINING TO THE PARTICULAR AUTOAUDIT TO BE USED FOR THE AUTOEVENT
            // return externalMonitoringHandler[extra.autoaudit].preprocessAutoEventDataStraightAway(data, extra);
            // MIGHT NOT EVEN BE NECESSARY, YOU CAN ALSO JUST return data;
            // THEREFORE THE SOURCE MUST MAKE SURE IT SENDS THE RIGHT ONE WITH RIGHT PROPERTIES
            // THEN IT ALSO MEANS YOU CAN EVEN DO SOME VALIDATIONS ON data BASED ON THE AUTOAUDIT
...
module.exports = ExternalAutoAuditingFunctions;
